...
import { createExecutionTool, createExplorationTools, } from "./sandbox-tools.js";
...
export class MCPSandboxExplorer {
    ...
    serverToolsMap = {};
    outputDir;
    constructor(sandboxProvider, servers, outputDir) {
    ...
    async generateFileSystem() {
        ...
        for (const server of this.servers) {
            try {
                ...
                console.error(`  ✗ Failed to fetch tools from ${server.name}: ${error instanceof Error ? error.message : String(error)}`);
                // Continue with other servers
            }
        }
        const totalTools = Object.values(this.serverToolsMap).reduce((sum, tools) => sum + tools.length, 0);
        if (totalTools === 0) {
        ...
        await writeFilesToSandbox(this.sandboxProvider, this.serverToolsMap, this.servers, this.outputDir);
     ...
     */
    getToolsSummary() {
        const servers = Object.entries(this.serverToolsMap).map(([name, tools]) => ({
            name,
    ...
    async cleanup() {
        ...
        try {
            ...
            console.error(`Warning: Error stopping sandbox: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
