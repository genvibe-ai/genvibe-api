 ...
 */
export function generateTypeScriptInterface(schema, interfaceName) {
 ...
 */
export function extractJSDocFromSchema(schema, toolName, toolDescription, serverName) {
    const lines = ["/**"];
    ...
    lines.push(" *");
    lines.push(" * @note Response format: The MCP tool returns an object, not an array.");
    lines.push(" *       For text responses, access the data via the returned object/string.");
    lines.push(" *       Example: const result = await tool(...); // result is an object or string");
    lines.push(" *       Always log the response first to understand its structure!");
    lines.push(" */");
    return lines.join("\n");
}
