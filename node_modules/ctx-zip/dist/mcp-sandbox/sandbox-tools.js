 ...
 */
export function createExplorationTools(sandboxProvider, serversDir) {
    return {
        sandbox_ls: tool({
            description: "List directory contents in the sandbox. Shows files and directories with details.",
        ...
        sandbox_cat: tool({
            description: "Read the contents of a file in the sandbox. Use this to view tool definitions and documentation. REQUIRED: You must provide the 'file' parameter with the full path to the file you want to read.",
            inputSchema: z.object({
                file: z
                    .string()
                    .describe("Full path to the file to read (REQUIRED). Example: '/vercel/sandbox/servers/README.md'"),
            }),
            async execute({ file }) {
        ...
        sandbox_grep: tool({
            description: "Search for a pattern in files within the sandbox. Useful for finding specific functions or text. REQUIRED: You must provide the 'pattern' parameter.",
            inputSchema: z.object({
        ...
        sandbox_find: tool({
            description: "Find files by name pattern in the sandbox. Use wildcards like *.ts to find TypeScript files. REQUIRED: You must provide the 'pattern' parameter.",
            inputSchema: z.object({
                pattern: z
                    .string()
 ...
 */
export function createExecutionTool(sandboxProvider, userCodeDir) {
    return {
        sandbox_exec: tool({
            ...
            inputSchema: z.object({
                code: z
                    .string()
                    .describe("TypeScript code to execute (REQUIRED). IMPORTANT: Use relative imports with .ts extension like '../servers/grep-app/index.ts' to import MCP tools."),
                filename: z
                    .string()
                    .optional()
                    .default("script.ts")
                    .describe(`Filename to save in ${userCodeDir}. Defaults to 'script.ts' if not provided.`),
            }),
            async execute({ code, filename }) {
                ...
                try {
                    ...
                    try {
                        ...
                        console.log(`[sandbox_exec] Completed in ${execTime}ms (exit code: ${result.exitCode})`);
                    }
                    catch (raceError) {
                        const execTime = Date.now() - execStartTime;
                        console.error(`[sandbox_exec] Timeout after ${execTime}ms: ${raceError instanceof Error
                            ? raceError.message
                            : String(raceError)}`);
                        throw raceError;
                    }
...
${stdout || "(no output)"}${stderr ? `\n\nWarnings/Info:\n${stderr}` : ""}`;
                    ...
                    console.error(`[sandbox_exec] ERROR: ${error instanceof Error ? error.message : String(error)}`);
...
Stderr: ${stderr || "(empty)"}`;
                    ...
                    return `⚠️ Execution error: ${error instanceof Error ? error.message : String(error)}${errorDetails}${capturedOutput ? `\n\nCaptured Output:\n${capturedOutput}` : ""}
...
This is a system-level error (not a code error). The script couldn't be executed. Check if the file was written correctly or if there's a sandbox issue.`;
        ...
        sandbox_write_file: tool({
            ...
            inputSchema: z.object({
                filename: z
                    .string()
                    .describe("Name of the file to write (e.g., 'helper.ts', 'utils.ts'). Will be created in user-code directory."),
            ...
            async execute({ filename, content }) {
                ...
                try {
                    ...
                    console.error(`[sandbox_write_file] ERROR: ${error instanceof Error ? error.message : String(error)}`);
        ...
        sandbox_edit_file: tool({
            ...
            inputSchema: z.object({
                filename: z
                    .string()
                    .describe("Name of the file to edit in user-code directory (e.g., 'script.ts')"),
                old_text: z
                    .string()
            ...
            async execute({ filename, old_text, new_text }) {
                ...
                try {
                    ...
                    console.error(`[sandbox_edit_file] ERROR: ${error instanceof Error ? error.message : String(error)}`);
